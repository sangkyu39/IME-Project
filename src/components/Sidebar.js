	...
	return (
		<SiderBarStyled>
			...
			<SectionWrap>
				...
				<SectionStyled onClick={onClickFeedBtn} style={{ color: "var(--grayscale-400, #7883A6)" }}>
					<div>
						<FeedBack />
					</div>
					피드백 보내기
				...
				<SectionStyled onClick={handleLogout} style={{ color: "var(--grayscale-400, #7883A6)" }}>
					<div>
						<Logout />
					</div>
					로그아웃
				...
				{pathname.includes("/admin") && (
					<>
						<SectionStyled
							...
							style={{
								color: getColorForSection("/admin/student-management", pathname),
								marginLeft: "48%",
								marginBottom: "1%",
								marginTop: "1%",
							}}>
...
const SectionStyled = styled.div`
	...
	width: 252px;
	height: 62px;
	background: ${(props) => (props.active ? "var(--primary-50, #FDE6EB)" : "var(--white, #fff)")};
	display: flex;
	align-items: center;
	justify-content: flex-start;
	margin-bottom: 40px;
